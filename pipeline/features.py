...
def extract_mfcc(y: np.ndarray, sr: int, n_mfcc: int = N_MFCC) -> np.ndarray:
    ...
    # 시간축(frames)을 평균내서 고정 길이 벡터로 만듭니다.
...
def extract_features(y: np.ndarray, sr: int) -> dict:
    ...
    features = {
        ...
        "zcr_mean": extract_zcr(y),
        "duration_ms": extract_duration_ms(y, sr),
        "rms_mean": extract_rms(y),
        "spectral_centroid_mean": extract_spectral_centroid(y, sr),
    }
