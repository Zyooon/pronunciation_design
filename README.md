...
현재 버전은 원어민 reference와 사용자 발음을 비교해 발음 점수를 계산합니다.  
앞으로는 단어 전체 점수보다 음소별 차이를 더 정확히 잡는 방향으로 개선할 예정입니다.

- `/i/`와 `/iː/`처럼 길이와 모음 유지 시간이 중요한 발음 구분 개선
- `/r/`와 `/l/`처럼 한국어 화자가 헷갈리기 쉬운 최소쌍 데이터 확장
- 음소가 단어의 앞, 중간, 끝 중 어디에 있는지에 따라 feature를 다르게 추출
- good / korean_like / wrong_or_noisy 샘플 평가 결과를 자동으로 리포트화

이를 통해 사용자가 어떤 발음을 왜 틀렸는지 더 쉽게 이해할 수 있는 피드백 시스템으로 확장할 예정입니다.